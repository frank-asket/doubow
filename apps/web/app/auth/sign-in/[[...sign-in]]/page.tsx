  ...
  if (!publishableKey) {
    return (
      <div className="rounded-xl border border-[0.5px] border-slate-200 bg-white p-5 text-sm text-[#3d4947] shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
        <p className="font-semibold text-[#171d1c] dark:text-slate-100">Clerk sign-in is not configured</p>
        <p className="mt-2 leading-relaxed">
          ...
          repo root <code className="rounded bg-slate-100 px-1.5 py-0.5 text-xs dark:bg-slate-800">.env</code> (see{' '}
          <code className="rounded bg-slate-100 px-1.5 py-0.5 text-xs dark:bg-slate-800">apps/web/.env.example</code>), then
          restart <code className="rounded bg-slate-100 px-1.5 py-0.5 text-xs dark:bg-slate-800">next dev</code>.
        </p>
      </div>
    )
  }
